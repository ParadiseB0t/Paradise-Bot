...
export default {
    ...
    options: [
        ...
        }, 
        {
            name: 'amount',
            type: 'NUMBER',
            description: 'The amount to set the money to',
            required: true,
        },
    ],
    ...
    callback: async ({ interaction, guild, }) => {
        ...
        if(which === 'global') {
            ...
            const embed = new MessageEmbed().setTitle('Global Admin Set Succesful').setDescription(`<a:tick:961700539475820614>  You have set ${user?.username}'s global account balance to ${config.currency}${amount}`).setColor('#feed83').setFooter('Paradise Beta').setTimestamp(Date.now())
            return embed
        }
        if(which === 'server') {
            eco.guildCheck(userId, guildId).then(async guildCheck => {
              ...
              const set = await eco.guildSet(userId, guildId, amount)
              console.log(set)
              const embed = new MessageEmbed().setTitle('Guild Admin Set Succesful').setDescription(`<a:tick:961700539475820614>    You have set ${user?.username}'s guild account to ${config.currency}${amount}`).setColor('#feed83').setFooter(`Paradise Beta | Version ${config.version}`).setTimestamp(Date.now())
              return embed
        }
    }
} as ICommand
